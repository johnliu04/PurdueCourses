...
treeNode* search(treeNode * tn, int value)
{
    ...
    treeNode * help = search(tn->leftChild, value);
    if (help == NULL){
      help = search(tn->rightChild, value);
    }
...
{
    treeNode * compar = needle;
    treeNode * contain = haystack;
   
     contain = search(contain, compar->value);
     if (contain == NULL){
       return false;
     }
     
     if (isSubTree(contain, compar)){
       return true;
     }
