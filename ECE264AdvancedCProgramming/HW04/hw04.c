...
{ 
  ...
  int dim = datapoint -> dimension; //read the value of dimension
  int i;
  long long int differ;

  for (i = 0; i < dim; i++)
  {
    differ = (datapoint -> data[i]) - (centroid -> data[i]);
    sum += differ * differ;
  }
...
int closestCentroid (int kval, DataPoint * datapoint, Centroid * *centroids)
{
  ...
  int i;
  long long int inidis;
  long long int dis;

  inidis = distance(datapoint, centroids[0]);

  for (i = 1; i < kval; i++)
  {
    dis = distance(datapoint, centroids[i]);
    if (inidis > dis)
    { 
      inidis = dis;
      mindex = i; 
...
void kmean (int kval, int nval, DataPoint * *datapoints, Centroid * *centroids)
{
	...
	while (mark){
		...
		for (int i = 0; i < nval && !initry; i++){
		    int compare = datapoints[i] -> cluster;
		    int minindex= closestCentroid(kval, datapoints[i], centroids);
		    datapoints[i] -> cluster = minindex;
		    if (compare == minindex){
		      mark--;
		    }
		}
