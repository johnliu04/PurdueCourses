...
void tty_init(void) {
    ...
    //set up the GPIO registers for pins PA9 and PA10
    RCC->AHBENR |= RCC_AHBENR_GPIOAEN;
    GPIOA->MODER |= GPIO_MODER_MODER9 | GPIO_MODER_MODER10;
    GPIOA->MODER &= ~(GPIO_MODER_MODER9_0 | GPIO_MODER_MODER10_0);

    //initialize the UART (the corresponding USART for PA9 and PA10) for 115.2Kb/s operation
    //with an 8-bit data word size, and one stop bit.
    GPIOA->AFR[1] |= 1<<(4*1) | 1<<(4*2);
    RCC->APB2ENR |= RCC_APB2ENR_USART1EN;
    //Disable the USART module (hint UE bit in CR1)
    ...
    //Wait for TEACK and REACK to be set by hardware in the ISR register
    while(!(USART1->ISR & USART_ISR_TEACK) || !(USART1->ISR & USART_ISR_REACK));
    //finally set the global variable ‘interrupt_mode’ to NON_INTR
...
void enable_tty_irq(void) {
    ...
    //It should also set the interrupt_mode flag to INTR.
    interrupt_mode = INTR;
}
...
static int getchar_nonirq(void) {
    // Student code goes here...
    //Check for the overrun flag, to check if we missed some characters.
    //If the overrun flag is set,
...
void USART1_IRQHandler(void) {
    ...
    if (USART1->ISR & USART_ISR_RXNE){
        //Read a character from the USART and pass it to insert_echo_char().
        insert_echo_char(USART1->RDR);
    }
    //If the TXE flat is set
...
static int getchar_irq(void) {
    // Student code goes here...
    //While the input_fifo does not contain a newline,
    while(!fifo_newline(&input_fifo)){
        //Wait for an interrupt
        asm("wfi");
    }
    //Return a character removed from input_fifo.
    return fifo_remove(&input_fifo);
}
...
static int putchar_irq(char ch) {
    ...
    if (USART1->ISR & USART_ISR_TXE){ //If the TXE interrupt enable bit is not set,
        USART1->CR1 |= USART_CR1_TXEIE;
        USART1_IRQHandler();
    }
    if (ch == '\n'){
